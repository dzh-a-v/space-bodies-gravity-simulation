...
└── program/
    ...
    │   ├── resources/textures/ Built-in planet PNG textures.
    │   └── ui/
    │       ├── app.py             Qt bootstrap.
...
- **Fragment layout** uses randomized non-overlapping placement inside the parent. Fragment radius targets `FRAGMENT_TARGET_VOLUME_FRACTION = 0.8` total parent volume, then binary-searches downward until all fragments fit fully inside the parent with a small surface gap. If even `MIN_RADIUS` fragments cannot fit, fragmentation returns `[]`.
